	...
	<div> 
		<?php
			try { 
                ...
                { 
					$email = DB::table('users')->where('username', 'John')->value('email');

					echo $email ;
					//DB::insert('insert into users (username, email, password) values (? ,? ,?)', ['john', 'john@admin.com', 'john@admin123']);
                   ...
                   // echo "Successfully connected to the database => " 
					...
					DB::table('videos')->where('status', false)->chunkById(100, function($users){
						foreach($users as $user){
							DB::table('videos')->where('id', $user-> id)-> update(['status'=> true]);
						}
